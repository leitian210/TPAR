...
class BaseModel(object):
    ...
    def train_batch(self,):
        ...
        for i in range(n_batch):
            ...
            scores = self.model(triple[:,0], triple[:,1], triple[:, 3])

            pos_scores = scores[[torch.arange(len(scores)).cuda(),torch.LongTensor(triple[:,2]).cuda()]]
            max_n = torch.max(scores, 1, keepdim=True)[0]
            loss = torch.mean(- pos_scores + max_n + torch.log(torch.sum(torch.exp(scores - max_n),1)))  # init 是  sum
    ...
    def evaluate(self, ):
        ...
        for i in range(n_batch):
            ...
            batch_idx = np.arange(start, end)
            subs, rels, objs, taus = self.loader.get_batch(batch_idx, data='test')
            scores = self.model(subs, rels, taus, mode='test').data.cpu().numpy()
            filters = []
            for i in range(len(subs)):
