...
class GNNLayer(torch.nn.Module):
    ...
    def forward(self, q_sub, q_rel, q_tau, hidden, edges, n_node, old_nodes_new_idx):
        ...
        message = hs + hr + h_hau
        alpha = torch.sigmoid(self.w_alpha(nn.ReLU()(self.Ws_attn(hs) + self.Wr_attn(hr) + self.Wqr_attn(h_qr) + self.Wtau_attn(h_hau))))
...
class TPAR(torch.nn.Module):
    ...
    def forward(self, subs, rels, taus, dynamic_KG, dynamic_M_sub, mode='train'):
        ...
        scores_all = []
        for i in range(self.n_layer):
            nodes, edges, old_nodes_new_idx = self.loader.get_neighbors(nodes.data.cpu().numpy(), dynamic_KG, dynamic_M_sub, mode=mode)
    
            hidden = self.gnn_layers[i](q_sub, q_rel, q_tau, hidden, edges, nodes.size(0), old_nodes_new_idx)
